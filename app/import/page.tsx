...
export default function ImportPage(){const [rows,setRows]=useState<ReturnType<typeof normalize>>([]);const [file,setFile]=useState("");const [busy,setBusy]=useState(false);const [msg,setMsg]=useState("");
...
return <main><header><div><div className="eyebrow">RACERS BASEBALL</div><h1>TrackMan Import</h1><p>Upload a TrackMan CSV or Excel export and preview the data before it enters the season dashboard.</p></div><a className="badge" href="/dashboard">← Coach Dashboard</a></header>
<section className="upload"><div className="uploadIcon"><Upload/></div><div><h2>Upload your CSV</h2><p>CSV, XLSX, and XLS files are supported. Your file is parsed in the browser in this build.</p><label className="button"><FileSpreadsheet size={18}/>{busy?"Importing…":"Choose CSV / XLSX"}<input type="file" accept=".csv,.xlsx,.xls" onChange={e=>{const f=e.target.files?.[0];if(f)upload(f)}}/></label>{file&&<span className="filename">{file}</span>}{msg&&<div className={msg.startsWith("Imported")?"success":"error"}>{msg.startsWith("Imported")?<CheckCircle2 size={17}/>:<AlertCircle size={17}/>} {msg}</div>}</div></section>
...
<div className="grid three"><Stat title="Average Exit Velocity" value={ev?ev.toFixed(1)+" mph":"—"}/><Stat title="Average Spin" value={spin?Math.round(spin).toLocaleString()+" rpm":"—"}/><Stat title="Cloud Database" value="Next build step"/></div>
